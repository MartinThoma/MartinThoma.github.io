...
medium_url: https://levelup.gitconnected.com/cross-site-scripting-xss-fd374ce71b2f
---
...
* XSS is part of the [OWASP Top-10](https://owasp.org/www-project-top-ten/), meaning it’s recognized as a common vulnerability. In the 2021 edition, it is part of A03 "Injection" ([source](https://owasp.org/Top10/A03_2021-Injection/))
...
* **2016**: A researcher found XSS vulnerability on Facebook ([source](https://whitton.io/articles/xss-on-facebook-via-png-content-types/))
...
<figure>
    <a href="../images/2020/10/xss-stored-cookie-theft.svg"><img src="../images/2020/10/xss-stored-cookie-theft.svg" alt="Four steps: attacker posts a script as a comment, the server stores it, a victim loads the page, the script sends the victim's cookie to the attacker" width="680" height="400" loading="lazy"></a>
    <figcaption>A stored XSS attack that steals a session cookie. Image by Martin Thoma</figcaption>
</figure>
...
Let’s see how an attacker can design a **cookie stealer** for **account
hijacking**.
...
another window. But it will call `https://attacker.com/steal` with a GET request
which has a cookie parameter. Hence, on the attacker's side, there only
...
doesn’t know the victim's credentials but has hijacked the session instead.

## What can I do to prevent XSS attacks?

Never blindly trust user data. Remove potentially harmful parts, escape parts
that will change the expected output. You might want to remove `<script>`
...
* Ruby: [ERB::Util.html_escape](https://ruby-doc.org/stdlib-2.6.3/libdoc/erb/rdoc/ERB/Util.html#method-c-html_escape)
* Rust: [htmlescape::encode_minimal](https://docs.rs/htmlescape/0.3.1/htmlescape/fn.encode_minimal.html)

Also, make sure you do that as early as possible. There should be a secure way
to use data from the database — and that should not require knowing that there
...
Remember to **sanitize ALL user inputs**, not only stuff that was in input fields. Don’t forget about [URL fragments](https://en.wikipedia.org/wiki/URI_fragment) and [Document.referrer](https://developer.mozilla.org/en-US/docs/Web/API/Document/referrer).
