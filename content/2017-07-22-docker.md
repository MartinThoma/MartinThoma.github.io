...
<dl>
    ...
    <dd>A virtual operating system. It is an instance of an image. A container
        is created with <code>docker run</code>. A metaphor used by
...
* `FROM` defines on which base image we build our image. We add another layer
  to that one.
...
* `WORKDIR /abs/path/to/workdir`: Work directory for any `RUN`, `CMD`,
...
You can find the data persistence volume with:
