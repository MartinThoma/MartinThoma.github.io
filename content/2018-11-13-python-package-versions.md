...
is not about what your version string should look like. It's about how to have
...
for "single-sourcing the package version". So it is a known problem with known
...
* Your package source could be simply copied, not installed. For example, if
  things run on AWS Lambda. In this case, there is no `setup.py` file. So all
...
don't think the first two look reliable enough to give them a try. And
...
<table>
    ...
    <tr>
        <td><a href="https://github.com/matplotlib/matplotlib">matplotlib</a></td>
        <td>versioneer</td>
        <td>?</td>
        <td>?</td>
    </tr>
