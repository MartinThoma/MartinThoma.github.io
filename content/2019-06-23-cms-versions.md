...
I want to store the history of how they changed over time. I wonder what the
...
If a page is edited, they have to insert into `revision` and into `text`. And there are some things like the title where changes are not tracked.
...
Creating backups can be done separately for text and all other tables. This can be an advantage if the amount of data is huge. Also, the [`text.old_flags`](https://www.mediawiki.org/wiki/Manual:Text_table#old_flags) can contain compression flags - meaning old revisions can be compressed for more storage efficiency while the current one can be plain text to be able to show it quickly.
