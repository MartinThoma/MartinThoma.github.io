...
ground truth. The cell $(i, j)$ tells how often class $i$ was
...
you can see the errors more easily. An awesome side-effect is that classes which look
...
$$E(C) = \sum_{i=1}^n \sum_{j=1}^n C_{ij} \cdot (i - j)^2$$
...
sub-structures which you destroy all the time. Instead of creating completely new
...
on how the atoms can be arranged and what is energetically preferable / more
...
In the algorithm, you could say that to make the score much worse (e.g. from 1234 to
...
```python
def optimize_simulated_annealing(c, steps=1000, temperature=1000, cooling_factor=0.995):
    # initialize
    best_permutation = [i for i in range(len(c))]
    best_score = calculate_score(best_permutation)
...
important than allowing blocks to be moved. So once in a while, I don't swap single
