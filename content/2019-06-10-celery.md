...
category: Code
tags: Inter-process Communication, Architecture, Python
...
let the user stare at a loading screen while you process the video. So simply
...
have the problem that the conversion can take arbitrarily long.

<figure>
    <a href="../images/2019/06/conversion-server-sync.png"><img src="../images/2019/06/conversion-server-sync.png" alt="Communication by a synchronous request with a conversion server" width="512" height="302"></a>
    <figcaption>Communication by a synchronous request with a conversion server</figcaption>
</figure>
...
<figure>
    <a href="../images/2019/06/conversion-server-polling.png"><img src="../images/2019/06/conversion-server-polling.png" alt="Communication by polling" width="512" height="486" loading="lazy"></a>
    <figcaption>Communication by polling</figcaption>
</figure>
...
the callback URL. Hence the name. It works like this:

<figure>
    <a href="../images/2019/06/conversion-server-callback.png"><img src="../images/2019/06/conversion-server-callback.png" alt="Communication by a callback" width="512" height="191" loading="lazy"></a>
    <figcaption>Communication by a callback</figcaption>
</figure>
...
don't scale well. Assume you suddenly go from several dozen users to tens of
...
do double the work with double the number of machines.
...
<figure>
    <a href="../images/2019/06/conversion-service-callback-api.png"><img src="../images/2019/06/conversion-service-callback-api.png" alt="A file conversion service based on callbacks" width="512" height="254" loading="lazy"></a>
    <figcaption>A file conversion service based on callbacks</figcaption>
</figure>
...
contains the upload form / the dashboard for the user and the workers.
...
Sometimes you send it to a topic, in other systems it is called a queue.
Consumers / clients can read / listen to those queues / topics.


## Celery

Celery is a task queue. While I'm not too deep into the topic, I would say the
main advantage of Celery compared to using the message brokers directly is
...
Of course, there is also the "web" service which I left out here to keep
it simple.
