...
* XXE attacks were number 4 in the **OWASP Top 10** (2017). In the 2021 edition, they are part of A05 "Security Misconfiguration" ([source](https://owasp.org/Top10/A05_2021-Security_Misconfiguration/))
...
But if you change the xml string, then you can read the list of users in `/etc/passwd`. Please note that since lxml 5.0, `resolve_entities` defaults to `'internal'`, so this example only works if you pass `resolve_entities=True` to the parser:
...
etree, minidom, pulldom, xmlrpc. According to the documentation, several of them
are vulnerable to attacks like the billion laughs attack or quadratic blowup
(external entities are not resolved by default with recent expat versions). Additionally, [lxml](https://lxml.de/) is widespread. It mentions that you
