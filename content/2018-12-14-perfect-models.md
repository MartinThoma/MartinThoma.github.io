...
\begin{cases}
    (x_1 > 0 \text{ AND } x_2 > 0) \text{ OR } (x_1 < 0 \text{ AND } x_2 < 0)&\text{orange}\\
    (x_1 > 0 \text{ AND } x_2 < 0) \text{ OR } (x_1 < 0 \text{ AND } x_2 > 0)&\text{blue}\\
\end{cases}
...
The circle dataset can be generated by drawing $r \in [0, 6]$ and $\theta \in [0, 2\pi)$.
...
Basically, it is choosing $r \in \mathbb{R}_{\geq 0}$. The $\alpha \in [0, 360)$ defines
...
more relevant, we don't have $r$. $r$ is basically the distance from $(0, 0)$.
...
Increasing the batch size leads to a smoother loss curve.
