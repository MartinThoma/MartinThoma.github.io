...
* **Information extraction**: For example, find the date in a calendar
  application when the user enters the name of the event. Or dates in an e-mail
  in order to allow users to create a calendar date.
...
* [Bing Liu Opinion Lexicon](http://www.cs.uic.edu/~liub/FBS/opinion-lexicon-English.rar)
    * 6789 words (2006 positive, 4783 negative)
* [SentiWordNet](http://sentiwordnet.isti.cnr.it)

Positivity of a word can be inferred from reviews. Reviews with many stars
...
the beginning of a named entity (B) or the continuation of a named entity (I):
...
in a corpus which contain "Albert Einstein" and "Ulm". You might find
patterns like:
...
* Statistical parsing
* Stemming: Bring a word into a normalized form (the stem). Mostly for verbs.
* Tokenization: Segment the text into tokens.
* Tokenizer: Splits a text into tokens.
* Viterbi Algorithm
* word2vec: Embed any word in a (high-dimensional) vector space. Allows vector arithmetic.

More might be in [my ML Glossary](../ml-glossary/).
...
when $c$ becomes "large" (depends on the dataset), just replace $N_c$ by a
best-fit power law.
...
can be followed / preceded by many different words, others (like "San", "Angelo", "D.C.", "United States of", ...) almost always appear next to the same few words.
