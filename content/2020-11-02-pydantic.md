...
For this article, I assume that your data is a network of people in [people.json](https://gist.github.com/MartinThoma/517d20998501afc4fff72be032782d41). They have an ID, a name, a list of friends given by their ID, a birthdate, and the amount of money in their bank account.
...
* **Ugly Any**: The type annotations have `Mapping[str, Any]`, because it was considered too complicated or even impossible to know which values the dictionary representing a person can map to.
* **Ugly indexing**: It’s just syntax, but `["id"]` is twice as long as `.id`. This is the reason why [bunch](https://pypi.org/project/bunch/)/[munch](https://pypi.org/project/munch/) exists.
* **Typos**: If you make a typo in any of the string indices of a dictionary, no static code analysis tool can help you to detect it. No auto-completion will work properly.
* **Late Errors**: Good software modularizes responsibilities. There is one module responsible for the business logic and one for input/output. When I write “module” I mean a unit of code — it could be a Python module, a class, a function, or a method. It is bad to have simply one function to deal with those different types of complexity. Getting the business logic right is inherently different from making sure your input/output looks as expected. One is about defining and validating proper interfaces, the other one is about understanding the domain.
* **Documentation**: New developers regularly have to read the code of bigger applications. Usually, there is no time to explain every single part in great detail by somebody who knows everything about the application. Most of the time, there isn’t even a single person who knows everything — even if the project was done by a single developer. I always forget parts of my code. Hence, documenting is crucial. In Python, documenting expected parameter types and return values is crucial. Having `Mapping[str, Any]` is better than nothing, but way worse than `Person`.
...
Please note that [datetime](https://pydantic-docs.helpmanual.io/usage/types/#datetime-types) and Decimal are automatically parsed — you should still always look up how it is done. Doing data validation early is good so that errors also happen early and in a known place. This means pydantic nudges you into a good design. I love it 😍
...
For immutable data types like strings, integers, floats, tuples, you can simply assign the value. For mutable ones, you need to use `Field` with a `default_factory` that generates a new list every time. [Learn why mutable defaults are evil](https://docs.python-guide.org/writing/gotchas/#mutable-default-arguments), if you don’t know it already.
...
Pydantic calls those **extras**. If you ignore them, the read pydantic model will not know them. **Ignored extra arguments are dropped**. Allowing them means accepting that this unfortunate design is necessary. **Allowed extras will be part of the parsed object**. Finally, **forbidding** extra arguments means a `pydantic.ValidationError` **exception** will be raised if an extra argument occurs.

This is configured by adding an inner class called `Config` to the pydantic model:
...
In our case, we might want to prevent people from being friends with themselves:
...
![Photo by [Science in HD](https://unsplash.com/@scienceinhd) on [Unsplash](https://unsplash.com)](../images/2020/11/pydantic-1.jpg)*Photo by [Science in HD](https://unsplash.com/@scienceinhd) on [Unsplash](https://unsplash.com)*

Property-based tests auto-generate inputs for the function under test and make sure a certain property is fulfilled. In the simplest case, this property is that the function under test does not crash. If you want to learn more about this type of testing, read my article about [property-based testing with hypothesis](../property-based-testing/).
...
![Photo by [ThisisEngineering RAEng](https://unsplash.com/@thisisengineering) on [Unsplash](https://unsplash.com)](../images/2020/11/pydantic-2.jpg)*Photo by [ThisisEngineering RAEng](https://unsplash.com/@thisisengineering) on [Unsplash](https://unsplash.com)*
...
* ✘ [PyPI project](https://pypi.org/project/pydantic/): Sadly, this only has one maintainer. I always like it a bit better if there are two. Simply in case one of them loses their password.

* ✔️ Self-decided project status: The maintainer considers pydantic to be production-ready/stable.

## Summary

pydantic is an awesome data parsing and validation library. It can help you a lot in getting better [type annotations in Python](../type-annotations/). Use it!
