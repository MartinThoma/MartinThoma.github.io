...
Slack and WhatsApp. If you keep the number of users low (e.g. less than 100)
and the number of features limited, then it is actually pretty easy to create
such a chat app.

I've created a small proof-of-concept chat app with Flask and jQuery. It runs in
the browser.
...
<figure>
    <a href="../images/2019/12/chat-flask.png"><img src="../images/2019/12/chat-flask.png" alt="Chat App" width="512" height="248"></a>
    <figcaption>Chat App</figcaption>
</figure>

We store the messages - and only that - in a single table in a SQLite database.
We have an API which has one endpoint <code>messages</code> which allows you to
...
production system this is an awful idea) and they store the latest message ID
they have seen.
...
See [this codepen](https://codepen.io/swaibu/pen/QxJjwN) for a nice theme
by Rumbiiha Swaibu.
...
* Registration and Authentication: It is important who writes something. So
  ...
  change the ID to somebody else's ID, or to have arbitrarily many IDs. It is hard to
...
* Chatrooms: People want to have private conversations. Conversations split
  by topic.
...
* Tenants: Besides chats / chatrooms, you might want to run the chat
  application as a service for organizations. They have their own (separate)
...
* Polling: Pinging the server twice per second is no big deal if you have few
