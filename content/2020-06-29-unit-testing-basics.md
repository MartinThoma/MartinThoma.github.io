...
We want to test these functions. I will explain the reasons for testing and what testing means later. For now, let’s just say we want to avoid programming errors.
...
Awesome! You can see that it took 0.03 seconds to execute. There are 5 dots after `test_math_functions.py`. Those indicate that 5 tests were executed and successful.
...
* **Breaking Changes**: For a bigger project, it is sometimes hard to have every part in mind. By writing tests, you make it easier to change something and see if / where things break. This does not only help you but also team members. Including ones that are not there yet.
...
I hope at this point we agree that having tests is a good idea. But how many tests do you need? When have you tested everything?
...
If you look at the Collatz function from above, there are 5 lines to test:
...
If I execute `next_collatz_element(4)`, then it will execute lines 1–3. Lines 4 and 5 will not be hit. This means a unit test like that could not detect an issue on line 4 or 5. It only covers 3 of 5 lines. One says that it has 60% line coverage.
...
* Get output to the terminal by adding `--cov-report term`
...
Happy tests, where you check the output of the tested function for a typical input, are even better. I call them *happy* because they test what you expect to get.
...
If you execute this directly, lines 17–20 will run the doctest. The doctest looks for `>>>` within the docstrings and executes whatever follows as if it was entered in the interactive console. The next line is then the output which is compared to the output of the program.
