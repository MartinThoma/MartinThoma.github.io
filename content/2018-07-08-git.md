...
category: Code
tags: Version Control, Software Engineering, Command Line
...
one stream. Instead, you will have different directions in which you go. One
day you work on feature A, then the next day you have to fix something,
but you already changed a bit - not everything needed to implement A. Hence, you need
to go back.
...
In the past, people did so by copy-and-paste. But then you end up with your file
...
They start every message with a prefix. Copied from this page, here is what their
messages look like:
...
I also suggest keeping the first line of those messages shorter than 80 characters.
...
compares the current version with the one before.



## git blame

[`git blame`](https://git-scm.com/docs/git-blame) gives you an overview of who
last edited which line. It can be used to "blame" somebody for a change. Or,
better, to find the person to ask why something was done the way it
currently is in the repository.

For a part of the `filters.py` in SciPy ndimage, it looks like this:
...
history. Thus, you need to force-push which will cause
...
When you are currently in a merge, git will tell you from time to time that
...
See my [Software Versioning Cheat Sheet](../software-versioning-cheat-sheet/#nice-diffs_1).
