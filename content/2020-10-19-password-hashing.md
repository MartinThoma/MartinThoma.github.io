...
the (hopefully) hashed passwords. If the same algorithm is applied to all
passwords without a salt, you can see which users have the same passwords. Extra information
...
next to the hashed password. The only reason to have that string is to make
the same plaintext password have different hash values.
...
and sometimes even a number of rounds to hash. There are a lot of possibilities
...
A key derivation function as implemented in Python's
