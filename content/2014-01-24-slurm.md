...
specify program calls that get executed as soon as all conditions
are met.
...
More concretely, I am working in the field of ASR - Automatic Speech Recognition.
In ASR you have models that include probabilities for ... well, let's
not get into detail here. The important part is that those models
...
when this script is started. This can be done on different computers
in parallel.
...
* `sbatch`: Run a batch script in the background
* `scancel`: Stop a job
...
Another important command is `squeue`. It allows you to list all jobs on the
