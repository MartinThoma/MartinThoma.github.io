...
## First steps
First, you have to install `swi-prolog`. After installing it, you can call it
with `swipl`.

When you've started `swipl`, you can import your `myscript.pl` files with `consult(myscript).`.
Don't forget the period at the end. It is important and somewhat like the
`;` in C.
...
* Note that names of all relationships and all objects have to begin with a
lowercase letter. Variables begin with a capital letter.
* The relationship is written first (`loves`). The objects are separated by `,`
...
`own(mary,book)` can be derived from the known facts. It is important that it
...
Source: [Wikipedia](https://en.wikipedia.org/wiki/Fox,_goose_and_bag_of_beans_puzzle)
...
* The only two things that the farmer can leave alone are the fox and the bag of beans.
...
* Define a rule `step(S1, Description, S2)` that makes one step from situation
  `S1` to situation `S2` where `Description` contains the information about what was
...
* the man and the beans / goose / fox go to the other side.
...
* Either you're already in the situation you want to get to. So you don't
  have to do any step and it doesn't matter which situations you have seen so far.
* Or you want to get from `S` to `G`. That is only possible if you can get
