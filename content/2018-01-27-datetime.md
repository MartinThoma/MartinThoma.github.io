...
---
Specifying a point in time is a pretty common task. And it is extraordinarily
difficult. Let's have a look at the different ways to specify a point in time.
...
all data related to this credit card user. Of course, most of it has time
stamps. As it is another system, the time has to be transferred from the
...
Especially the last point is a killer criterion against Unix time in many
applications.
...
human-readable, unambiguous format that looks like this:
...
* <span style="color: red;">&#8861;</span> ISO 8601 does not only have
  ...
  `20180127T164501Z` where Z means Zero offset for UTC. So there are multiple
...
* A given geographic point (e.g. a city) always has the same time zone.
...
gives `2018-01-27T22:51:17.691Z`. So it is an ISO 8601 datetime with milliseconds
