...
method of `xs`. It never assigns a value to `xs`. The second one is equivalent to:
...
When Python parses the assignment `xs = ...`, `xs` is assigned to the local
scope. But in the local scope, `xs` does not exist before `xs = xs + [42]` is
executed. Hence the error.

In the first example with `xs.append(42)`, the global `xs` is used.
...
read the `bool(external_service()) == True` path and assumed that `y` would
be an integer.
...
You can see that mypy assumes that `y` should be the same type in both cases. It’s
...
That should have been expected. Assigning `None` to a type which does not include
`None` is not possible — and adding None to that type might cause many more
...
But I can also see why this feels strange. What might be cleaner is this:
...
We’ve seen that you can access globals with the `global` keyword or via the
