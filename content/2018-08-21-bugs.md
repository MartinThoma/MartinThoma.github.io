...
category: Code
tags: Software Engineering, Bugs, Python
...
An example would be this `sum.py` executed with Python 3:
    ...
    n1 = input()
    n2 = input()
    print("The sum of those two numbers is {}".format(n1 + n2))
...
I would like to give you some ideas of which types of bugs can happen and - if
possible - how to prevent them.

### Functional Bugs

**Off by one errors** are super common: `<` or `<=`? `>` or `>=`? Smaller or
greater? They are logic errors which can often be
caught by calling the function with example input and checking the
actual output against known example output. Those off-by-one errors can also
...
by using unit libraries like [Python's Pint](https://pint.readthedocs.io/en/latest/).
...
account. You have one process P-in which receives incoming transactions and
...
You have 3141 EUR on that account identified by 'Martin'. At midnight in the
...
constant, but it depends a lot on the use case if this is valid.

**Uninitialized resources** (e.g. `NullPointerException` or `KeyError` in Python)
...
Another one I see often with browsers is that they are **too memory intensive**.
...
That doesn't help. What does help are answers to the following questions:

1. What did you do?
2. What did you expect to happen?
3. What actually happened?

Ideally, you can figure out a minimal example to reproduce it.

If you have a stack trace or some error code, that is good as well.
...
deleting stuff permanently), then fixing any bug related to those has a high risk.
