...
how likely it is (under the assumption of a hypothesis $H_0$) to get more
extreme results.
...
* **Define a hypothesis and an alternative**: For example $H_0: \mu = 100$ and
  ...
  the IQ.
* **Control errors**: You can make two errors. Either $H_0$ is true and you
...
* **Test statistic**: You have a value which indicates something about the parameters
  in the hypotheses. For example $T(X_1, \dots, X_n) = \frac{1}{n}\sum_{i=1}^n X_i$.
...
* **Calculate test decision**: Hence you can calculate a $c \in \mathbb{R}$ such that
  $P_{H_0}(H_0 \text{ is rejected}) = P_{H_0}(T \geq c) \leq \alpha$.
  It does not have to be $T \geq c$, but often it is.
...
If $p^* \leq \alpha$, then $H_0$ can be rejected at level $\alpha$.


## Interesting statements

I just came across the following statements which I think are interesting enough
to share them. **All of them are wrong.**

The following was taken from a German statistics exam by Dr. Klar (KIT, WS 2013/2014).
